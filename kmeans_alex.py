...
def get_data(file_name):
    with open(file_name) as file:
        ...
        titles = []
        ids = []
        TITLE_WEIGHTING = 7  # Keywords in the title are X times more important than those in the text
        for norme in liste_normes:
            formatted_title = " ".join([st.stem(w) for w in word_tokenize(norme['name']) if w not in stop_words])
            texts.append((formatted_title + ' ')*TITLE_WEIGHTING
...
# Retreive data
iso_texts, iso_ids, iso_titles = get_data('ISO_standards.yml')
nist_texts, nist_ids, nist_titles = get_data('NIST_standards.yml')
...
for k in range(len(iso_texts)):
    ...
    cv = CountVectorizer(ngram_range=(1, 2), min_df=0.001, max_df=0.75)  # max_df set to eliminate most common words (like 'sec')
    cv_matrix = cv.fit_transform(texts).todense()

    # Create Kmeans model
    NUM_CLUSTERS, MIN_CLUSTER_SIZE, MAX_CLUSTER_SIZE = 40, 2, 4
    model = KMeansConstrained(n_clusters=NUM_CLUSTERS, size_min=MIN_CLUSTER_SIZE, size_max=MAX_CLUSTER_SIZE, max_iter=10000,
                              n_init=100, random_state=42)  # Initializing KMeans model
...
with open('mapping.csv', 'w', encoding='UTF8') as f:
